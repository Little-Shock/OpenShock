...
- `Issue -> Session` 默认是 `1:N`，且由系统自动维护
- 一个 `Issue` 允许同时存在多个活跃 `Session`
- `PR` 不强制一对一绑定 `Session`
- 更推荐的交付单元是 `Room` / `Workroom`：一组相关 Session 的结果汇聚成一个 PR 或一批 PR，交付后 Room 归档

### 2. Runtime 与 Provider

- provider 能力默认来自机器上已安装的本地 CLI
- Agent 只负责偏好选择和允许范围覆盖
- OpenShock 复用用户现有 Codex / Claude Code 等本地 CLI 配置

### 3. Inbox、Mailbox 与发言规则
