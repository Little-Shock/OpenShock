...
- 角色拓扑不只做静态预览，还要围当前 room / run truth 派生默认下一棒 governed handoff；当前一棒完成时应允许 auto-advance 到下一棒；final lane done 时要显式回链 delivery entry / closeout，并继续派生最终 delivery delegate / inbox signal / delegated closeout handoff；delivery delegate 还要支持 `formal-handoff / signal-only / auto-complete` automation policy；delegated closeout 的 formal comment 与 lifecycle 也必须同步回 PR detail / related inbox；缺少合法接收方时必须显式 blocked
