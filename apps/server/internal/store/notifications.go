...
func authNotificationSignals(snapshot State) []notificationSignal {
	...
	for _, member := range auth.Members {
		if member.Status == workspaceMemberStatusInvited {
			signals = append(signals, notificationSignal{
				...
				Summary:       fmt.Sprintf("%s 已被邀请加入工作区；首次登录后可继续在账号中心完成后续步骤。", defaultString(member.Email, notificationMemberLabel(member))),
				Action:        HrefTargetLabel("/access"),
				Href:          "/access",
		...
		if member.PasswordResetStatus == authPasswordResetPending {
			signals = append(signals, notificationSignal{
				...
				Summary:       fmt.Sprintf("%s 正在等待完成密码重置；下一步需要在另一设备完成恢复后回到账号中心。", defaultString(member.Email, notificationMemberLabel(member))),
				Action:        HrefTargetLabel("/access"),
				Href:          "/access",
	...
	if auth.Session.EmailVerificationStatus != authEmailVerificationVerified {
		signals = append(signals, notificationSignal{
			...
			Summary:       fmt.Sprintf("%s 还没完成邮箱验证；验证后可继续使用账号中心里的恢复和登录流程。", defaultString(member.Email, notificationMemberLabel(member))),
			Action:        HrefTargetLabel("/access"),
			Href:          "/access",
	...
	if auth.Session.DeviceAuthStatus != "" && auth.Session.DeviceAuthStatus != authDeviceStatusAuthorized {
		signals = append(signals, notificationSignal{
			...
			Summary:       fmt.Sprintf("%s 当前还是 %s；通知会继续把这条阻塞提醒带回账号中心。", defaultString(member.Email, notificationMemberLabel(member)), notificationRecoveryLabel(auth.Session.RecoveryStatus)),
			Action:        HrefTargetLabel("/access"),
			Href:          "/access",
