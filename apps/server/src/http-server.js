  ...
  return http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (route.route === "V1_GET_TOPIC_EVENTS") {
        ...
        if (eventType && !CONTROL_EVENT_TYPES.has(eventType)) {
          throw new CoordinatorError("invalid_event_type", `unsupported event_type filter: ${eventType}`);
        }
        const allEvents = getTopicEvents(route.topicId);
        const filtered = eventType
          ? allEvents.filter((eventItem) => eventItem.event_type === eventType)
          : allEvents;

        const page = paginate(filtered, {
          cursor: parsedUrl.searchParams.get("cursor"),
          limit: parsePageLimit(parsedUrl.searchParams),
          scope: `topic:${route.topicId}:events:${eventType ?? "all"}`
        });

        sendJson(response, 200, {
          items: deepClone(page.items),
          page: page.page,
          stable_event_types: Array.from(CONTROL_EVENT_TYPES.values()),
          request_id: requestId
        });
        return;
      }
